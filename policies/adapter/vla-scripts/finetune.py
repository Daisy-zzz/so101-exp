...
class FinetuneConfig:
    ...
    action_from_pretrain: bool = False                      # If True, pretrains action head before finetuning entire model
    action_pretrain_path: str = "pretrained_models/libero_90_action_pretrain"         # Path to action head pretraining checkpoint
    # LoRA
    ...
    # Logging
    wandb_entity: str = "daisyzzz"          # Name of WandB entity
...
def run_forward_pass(
    ...
    if not (use_l1_regression):
        ...
        predicted_token_ids = action_head.module.predict_action(
            multi_layer_hidden_states,
            proprio=batch["proprio"] if use_proprio else None,
            proprio_projector=proprio_projector if use_proprio else None,
            phase=cfg.phase,
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    if cfg.use_minivlm:
        hf_token = "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"
    ...
    if cfg.action_from_pretrain:
        from safetensors.torch import load_file
        pretrained_vla_path = os.path.join(cfg.action_pretrain_path, "model.safetensors")
        pretrained_vla = load_file(pretrained_vla_path)
        for name, param in vla.named_parameters():
            if "action_queries" in name:
                vla.state_dict()[name] = pretrained_vla[name]
        print('loaded pretrained vla!!!!!!!!!!')
    ...
    if cfg.use_proprio:
        ...
        if cfg.action_from_pretrain:
            pretrained_proprio_projector_path = os.path.join(cfg.action_pretrain_path, "proprio_projector--checkpoint.pt")
            state_dict = torch.load(pretrained_proprio_projector_path, weights_only=True, map_location="cpu")
            proprio_projector.load_state_dict(state_dict)
            print('loaded pretrained proprio projector!!!!!!!!!!')
    ...
    if cfg.use_l1_regression:
        ...
        action_head._save_to_state_dict
        if cfg.action_from_pretrain:
            pretrained_action_head_path = os.path.join(cfg.action_pretrain_path, "action_head--checkpoint.pt")
            state_dict = torch.load(pretrained_action_head_path, weights_only=True, map_location="cpu")
            action_head.load_state_dict(state_dict)
            print('loaded pretrained action head!!!!!!!!!!')
    else:
        action_head = None
    ...
    # print(f"vla trainable params: {sum(p.numel() for p in trainable_params)}")
    if cfg.use_l1_regression:
